...
// Sage's code for assignment 8 (plus the edited keypress event function)
var pressCount = 0;
var interval = undefined;
var intervalTime = 500;
selectItem("#button5"); // set start at 5 (middle button)
function pressCountAction() {
	if (pressCount === 1) {
		selectPrevious();
...
$(document).keypress(function(event) {
	if (event.key == "a") {
		event.preventDefault(); // make sure page doesn't do something expected
		if (interval === undefined) {
			interval = setInterval(function() { pressCountAction() }, intervalTime)
		}
		pressCount++;
	}
